...
// the entire engine, transmission, brakes and shit, and weapons from hellll and i don't know what else is in this code. 5% of the code doesn't even work. :)
...
public class carController : MonoBehaviour
{
    ...
    public Vector2 frictionMultiplier;
    ...
    void moveCar()
    {
        ...
        if (carDriveType == driveType.AllWheelDrive)
        {
            ...
            leftWheelTorque = outputTorque * (1 - Mathf.Clamp(differentialTorqueDrop * ((steerAngle < 0) ? -steerAngle : 0), 0, 1.0f));
            rightWheelTorque = outputTorque * (1 - Mathf.Clamp(differentialTorqueDrop * ((steerAngle > 0) ? steerAngle : 0), 0, 1.0f));
    ...
    void adjustSidewaysFriction()
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            swf.stiffness *= frictionMultiplier.x;
            wheelColliders[i].sidewaysFriction = swf;
        }

        for (int i = 2; i < 4; i++)
        {
            swf.stiffness *= frictionMultiplier.y;
            wheelColliders[i].sidewaysFriction = swf;
        }
    }
